...
export function Sidebar() {
  ...
  return <aside style={{ width: 244, minHeight: '100vh', padding: '28px 16px', background: 'var(--surface-container)', borderRight: '1px solid var(--outline)', display: 'flex', flexDirection: 'column', gap: 8 }}>
    <Link href="/" style={{ textDecoration: 'none', color: 'var(--on-surface)', padding: '12px 14px 32px', display: 'block' }}>
    ...
    <nav aria-label="メインナビゲーション" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {NAV_ITEMS.map(({ label, href, icon: Icon }) => { const active = pathname === href; return <Link key={href} href={href} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 16px', borderRadius: 999, color: active ? 'var(--accent-text)' : 'var(--on-surface-variant)', background: active ? 'var(--primary-container)' : 'transparent', fontWeight: active ? 800 : 600, textDecoration: 'none', transition: 'background .2s' }}><Icon size={19} strokeWidth={active ? 2.6 : 2} />{label}</Link>; })}
    </nav>
