...
import { ANIME_STATUS_STYLES, HybridAnime } from '@/types/anime';
...
export function AnimeCard({ anime, showProgress = false, index = 0, onNavigate }: AnimeCardProps) {
  ...
  const statusStyle = status ? ANIME_STATUS_STYLES[status] : undefined;
  return <motion.div initial={reducedMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={reducedMotion ? undefined : { opacity: 0, scale: .98 }} transition={{ duration: reducedMotion ? 0 : .3, ease: 'easeOut', delay: reducedMotion ? 0 : Math.min(index * .025, .3) }} style={{ position: 'relative', zIndex: isHovered ? 2 : 1 }}>
    <Link href={`/anime/${anime.id}`} onClick={onNavigate} style={{ textDecoration: 'none' }} aria-label={`${anime.title}の詳細を見る`}>
      <motion.article onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} whileHover={reducedMotion ? undefined : { y: -6, scale: 1.015 }} transition={{ duration: reducedMotion ? 0 : .3, ease: 'easeOut' }} style={{ overflow: 'hidden', position: 'relative', aspectRatio: '3 / 4', borderRadius: 22, background: 'var(--surface-variant)', boxShadow: 'var(--card-shadow)', border: '1px solid color-mix(in srgb, var(--outline) 30%, transparent)' }}>
        <RemoteImage src={anime.image_url} alt={anime.title} sizes="(max-width: 768px) 42vw, (max-width: 1200px) 20vw, 240px" />
        {status && <span style={{ position: 'absolute', top: 10, right: 10, background: statusStyle?.background || '#49454f', color: statusStyle?.color || '#fff', padding: '5px 9px', borderRadius: 999, fontSize: 10, fontWeight: 800, boxShadow: '0 2px 8px rgba(0,0,0,.22)' }}>{status}</span>}
        <div style={{ position: 'absolute', inset: '35% 0 0', background: 'linear-gradient(transparent, rgba(29,27,32,.93))' }} />
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '14px 13px', minHeight: '58%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', background: 'linear-gradient(transparent, rgba(20, 16, 8, .96) 18%)', color: '#fff', opacity: isHovered ? 1 : 0, transform: isHovered ? 'translateY(0)' : 'translateY(18px)', transition: reducedMotion ? 'none' : 'opacity .3s ease, transform .3s ease', pointerEvents: 'none', zIndex: 3 }}>
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 13, color: '#fff', opacity: isHovered ? 0 : 1, transition: reducedMotion ? 'none' : 'opacity .3s ease', zIndex: 2 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginTop: 7, minHeight: 14 }}>{rating > 0 ? <StarRating value={rating} size={10} readonly /> : <span style={{ fontSize: 10, opacity: .82 }}>{anime.season || '作品情報'}</span>}<span style={{ fontSize: 10, opacity: .82, whiteSpace: 'nowrap' }}>{total > 0 ? `全${total}話` : '話数不明'}</span>{showProgress && status === '視聴中' && total > 0 && <span style={{ fontSize: 10, fontWeight: 800, whiteSpace: 'nowrap' }}>{progress}/{total}話</span>}</div>
